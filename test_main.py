...
#rois = IppgSignalObtainer.extractSeriesRoiRGBFromVideo(path, 30, 60, 0, True, True, False, True)
roi_signal = IppgSignalObtainer.extractFullFaceSkinRGBFromVideo(path, 30, 60, 0, False, 2, False)
ippg = IppgSignalObtainer.GetRppGSeriesfromRGBSeries(roi_signal["face_skin"]["red"], roi_signal["face_skin"]["green"], roi_signal["face_skin"]["blue"], 30, normalize=False, derivative=False, bandpass=False, detrend=True, method='pos')
plt.plot(ippg)
plt.show()
hr = calc.ObtainHeartRate(np.array(ippg), [], fs = 30, method='two_peaks_fft')
print(f"HR full face: {hr}")
"""
for roi in rois:
    print(f"{roi} ========================================================================")
    red = rois[roi]['red']
